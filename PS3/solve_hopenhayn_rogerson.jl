...
@with_kw mutable struct results 
    ...
    x_pr::Vector{Int64} # enter/exit choice 
end 
...
function bellman(prim::primitives, res::results)
    ...
    @unpack p, W_i, x_pr = res
    
    W_i_pr = zeros(ns) 
    for s_index = 1:ns
        s = s_grid[s_index]
        trans_probs = Π[s_index, :]
        W_i_pr[s_index] = π_incumbents(θ, c_f, p, s) + β*max(0.0, trans_probs'*W_i)
        x_pr[s_index] = 1 - (argmax([0.0 , trans_probs'*W_i]) - 1 )
    end 
    return W_i_pr
end 
...
function invisible_hand(res::results, prim::primitives; tol = 0.1)
