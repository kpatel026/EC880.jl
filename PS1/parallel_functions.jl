...
@everywhere @with_kw struct Primitives
    ...
    nz::Int64 = 2
    Π = SharedArray{Float64}([0.977 0.023 ; 0.074 0.926]) # productivity transition matrix 
end
...
@everywhere function Initialize()
    prim = Primitives()
    val_func = SharedVector{Float64}(zeros(prim.nk,prim.nz))
    pol_func = SharedVector{Float64}(zeros(prim.nk,prim.nz))
...
@everywhere function Bellman(prim::Primitives, res::Results)
    ...
    v_next = SharedVector{Float64}(nk,nz)
